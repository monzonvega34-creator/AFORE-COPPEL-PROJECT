...
<div style="font-size:25px; line-height:1.5; color:#1a1a1a; text-align:justify;">
  <p>
    ...
    hacia una estructura empresarial integrada por tres segmentos estratégicos complementarios, consolidándose como un conglomerado:
    tiendas, banco y afore. <b>Afore Coppel</b>, fue creada en <b>2005</b>,
...
<div style="font-size:25px; line-height:1.5; color:#1a1a1a; text-align:justify;">
  <p>
    Los recursos administrados en el <b>Sistema de Ahorro para el Retiro (SAR)</b> superan actualmente los <b>8.3 billones de pesos</b>. Dentro de este mercado, <b>Afore Coppel</b> se posiciona en el <b>cuarto lugar</b> en recursos administrados; sin embargo, ocupa apenas el <b>décimo lugar</b> en aportaciones voluntarias.
    <br><br>
...
df_recursos_copia = hoja["09_recursos"][[
    ...
    "monto_recursos_administrados_afores",
]]
...
df_ultimo = df_ultimo[df_ultimo["afore"] != "pensiones bienestar"]  

df_ultimo["monto_ahorro_voluntario_y_solidario"] *= 1_000_000
df_ultimo["monto_recursos_administrados_afores"] *= 1_000_000

orden_afores = (
    df_ultimo.sort_values("monto_recursos_administrados_afores", ascending=False)["afore"].tolist()
)
df_melted = df_ultimo.sort_values("monto_recursos_administrados_afores", ascending=False).melt(
    id_vars="afore",
    value_vars=["monto_ahorro_voluntario_y_solidario", "monto_recursos_administrados_afores"],
...
df_melted["tipo"] = df_melted["tipo"].map({
    ...
    "monto_recursos_administrados_afores": "Recursos Registrados SAR",
})
...
fig6_rec = px.bar(
    ...
    labels={"monto": "Monto", "afore": "AFORE", "tipo": ""},
)
...
fig6.update_layout(
    template="plotly_white",
    title=dict(
        text="<b>RENDIMIENTO SIEFORES COPPEL </b>",
        font=dict(size=FS_TITLE, color="#1a1a1a"),
        x=0.5, xanchor="center",
    ),
    xaxis=dict(
...
<div style="font-size:25px; line-height:1.5; color:#1a1a1a; text-align:justify;">
  <p>
    ...
    <br><br>
    Finalmente, la última gráfica muestra el <b>incremento mensual total de aportaciones voluntarias</b> acumulado a lo largo de todos los años, revelando patrones estacionales claros: una <b>caída en abril</b> asociada al período de declaración anual —donde las aportaciones voluntarias no generan un incentivo fiscal inmediato—
    <br><br>
    y <b>picos pronunciados en noviembre, mayo y enero</b>, meses en los que los cuentahabientes concentran sus aportaciones, posiblemente motivados por el cierre del ejercicio fiscal y la disponibilidad de liquidez extraordinaria como PTU o aguinaldo.
    <br><br>
    Esto confirma que el siguiente paso estratégico es <b>fortalecer y consolidar los canales digitales</b> para capitalizar este crecimiento estacional.
  </p>
</div>
""", unsafe_allow_html=True)
...
fig11 = go.Figure(go.Funnel(
    ...
    textfont=dict(size=30, color="white", family="Arial Bold"),
...
fig11.update_layout(
    title=dict(
        text="<b>Funnel de Conversión AFORE Coppel</b><br><sub>Millones de cuentas</sub>",
        x=0.5,
        xanchor="center",
        xref="paper",  
        font=dict(size=30, family="Arial Black", color="#0033A0"),
    ),
    funnelmode="stack",
    height=750,
    ...
    yaxis=dict(
        tickfont=dict(size=30, family="Arial Bold", color="#333333")  
    )
)
...
app = pd.read_csv("app_afore.csv")
app = app.sort_values(["afore", "fecha"])  # o la columna que agrupe cada serie
app["flujo_neto"] = app.groupby("afore")["monto_ahorro_voluntario_y_solidario"].diff()
app = app.dropna(subset=["flujo_neto"])  

colores   = {0: "#1E3FBE", 1: "#F5C800"}
etiquetas = {0: "Afore Coppel (sin app)", 1: "Con App"}

fig12 = go.Figure()
for valor in [0, 1]:
    datos = app[app["app_dummy"] == valor]["flujo_neto"].dropna()
    fig12.add_trace(go.Box(
        y=datos,
        name=etiquetas[valor],
        marker_color=colores[valor],
        boxmean=True,
        boxpoints="all",       # muestra todos los puntos
        jitter=0.5,            # dispersión horizontal de los puntos
        pointpos=0,            # 0 = centrado sobre la caja
        marker=dict(size=4, opacity=0.4, color=colores[valor]),
        line=dict(color="black"),
        fillcolor=colores[valor],
        opacity=0.6,
        hovertemplate=f"<b>{etiquetas[valor]}</b><br>flujo: $%{{y:,.2f}} Millones<extra></extra>"
    ))

fig12.update_layout(
    template="plotly_white",
    title=dict(
        text="Flujo neto del saldo según disponibilidad de app móvil",
        x=0.5, xanchor="center"
    ),
    xaxis_title="App Dummy",
    yaxis=dict(title="Flujo neto mensual (Millones de pesos)", tickprefix="$"),
    showlegend=False,
    height=900,
    width=1700,
)
...
fig13.update_layout(
    ...
    yaxis=dict(
        title=dict(text="Incremento Total (%)", font=dict(size=FS_AXIS)),
        tickfont=dict(size=FS_TICK),
        ticksuffix="%",
        showgrid=True,
        gridcolor=GRID_COLOR,
...
resumen_mes = (
    df2.groupby("mes")["incremento_mensual"]
    .sum()
    .reset_index()
    .rename(columns={"incremento_mensual": "incremento_incremento"})
)
resumen_mes["mes_nombre"] = resumen_mes["mes"].map(meses_nombres)

fig14 = go.Figure(go.Bar(
    x=resumen_mes["mes_nombre"],
    y=resumen_mes["incremento_incremento"],
    marker=dict(
        color=resumen_mes["incremento_incremento"],
        colorscale=[[0, "#1E3FBE"], [1, "#F5C800"]],
        showscale=False,
        line=dict(color="white", width=1)
    ),
    text=resumen_mes["incremento_incremento"].apply(lambda v: f"{v:.2f}%"),
    textposition="outside",
    textfont=dict(size=12),
    hovertemplate="<b>%{x}</b><br>incremento: %{y:.2f}%<extra></extra>"
))
fig14.update_layout(
    template="plotly_white",
    title=dict(
        text="<b>Incremento Mensual por Mes</b><br><sup>incremento de todos los años</sup>",
        x=0.5,
        xanchor="center",
...
graficas_2 = [
    ...
    ("Distribución del flujo Mensual de aportaciones voluntarias", fig12),
